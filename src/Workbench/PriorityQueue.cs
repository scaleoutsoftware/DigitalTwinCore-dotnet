...
// Remove the warnings about nullable properties without a nullable context
#nullable enable

// The PriorityQueue class implementation is adopted based on the original source code here:
...
namespace Scaleout.DigitalTwin.Workbench
{
    internal sealed class PriorityQueueDebugView<TElement, TPriority>
    {
        ...
        public (TElement Element, TPriority Priority)[] Items
        {
            get
            {
                List<(TElement Element, TPriority Priority)> list = new List<(TElement Element, TPriority Priority)>(_queue.UnorderedItems);
                if (_sort)
                {
