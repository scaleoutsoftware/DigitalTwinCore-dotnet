...
namespace Scaleout.DigitalTwin.Workbench
{
    internal class RealTimeProcessingContext : ProcessingContext
    {
        ...
        public override SendingResult RemoveRealTimeTwin(string targetTwinModel, string targetTwinId)
        {
            ...
            bool foundModel = _env.Instances.TryGetValue(targetTwinModel, out var instances);
            if (!foundModel)
                throw new KeyNotFoundException($"Model {targetTwinModel} not found. Register it first with the {nameof(RealTimeWorkbench)} before trying to remove an instance of this model's type.");
