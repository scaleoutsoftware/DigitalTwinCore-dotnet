    ...
    public class SimTimers
    {
        ...
        class StartSingleTimerProcessor : SimulationProcessor<SimulatedCarModel>
        {
            ...
            public override Task<ProcessingResult> ProcessModelAsync(ProcessingContext context, SimulatedCarModel digitalTwin, DateTimeOffset currentTime)
            {
                if (!_timerStarted)
                {
                    context.StartTimer("foo", TimeSpan.FromSeconds(10), TimerType.OneTime, TimerFiredAsyncHandler);
                    _timerStarted = true;
                }
        ...
        class StartRecurringTimerProcessor : SimulationProcessor<SimulatedCarModel>
        {
            ...
            public override Task<ProcessingResult> ProcessModelAsync(ProcessingContext context, SimulatedCarModel digitalTwin, DateTimeOffset currentTime)
            {
                if (!_timerStarted)
                {
                    context.StartTimer("foo", TimeSpan.FromSeconds(5), TimerType.Recurring, TimerFiredAsyncHandler);
                    _timerStarted = true;
                }
                return Task.FromResult(ProcessingResult.DoUpdate);
            }
        ...
        class TimerTwin : DigitalTwinBase
        {
            ...
            public override void Init(string id, string model, InitContext initContext)
            {
                ...
                initContext.StartTimer("foo", TimeSpan.FromSeconds(5), TimerType.OneTime, TimerFiredAsyncHandler);
            }
